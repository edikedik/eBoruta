...
class IterationAdjuster:
    ...
    def __call__(
            self, estimator: _E, features: Features,
            dataset: Dataset, trial_data: TrialData, **kwargs) -> CallbackReturn:
        num_features = len(features.tentative) * 2
        new_num_features = max([self.min_value, self.reducer(num_features)])
        estimator = change_params_and_reinit(estimator, {self.param_name: new_num_features})
...
class CatFeaturesSupplier:
    ...
    def __call__(
        ...
        if hasattr(estimator, 'cat_features'):
            ...
            estimator = change_params_and_reinit(estimator, {'cat_features': cat_features})
        return estimator, features, dataset, trial_data, kwargs
